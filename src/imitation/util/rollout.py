...
@dataclasses.dataclass(frozen=True)
class TrajectoryWithRew(TrajectoryNoRew):
  rews: np.ndarray
  """Reward, shape (trajectory_len, )."""


@dataclasses.dataclass(frozen=True)
class TransitionsNoRew:
  """A batch of obs-act-obs-done transitions.

  Often generated by combining and processing several TrajectoryNoRew objects
...
@dataclasses.dataclass(frozen=True)
class TransitionsWithRew(TransitionsNoRew):
...
def unwrap_traj(traj: TrajectoryWithRew) -> TrajectoryWithRew:
...
def calc_rewards_traj(traj: TrajectoryNoRew, reward_fn: common.RewardFn,
                      ) -> np.ndarray:
...
class TrajectoryAccumulator:
  ...
  def finish_trajectory(self, key: Hashable = None) -> TrajectoryWithRew:
    ...
    traj = TrajectoryWithRew(**out_dict_stacked)
  ...
  def add_steps_and_auto_finish(self,
                                ...
                                rews: np.ndarray,
                                dones: np.ndarray,
                                infos: List[dict]) -> List[TrajectoryWithRew]:
...
GenTrajTerminationFn = Callable[[Sequence[TrajectoryWithRew]], bool]
...
def min_timesteps(n: int) -> GenTrajTerminationFn:
  ...
  def f(trajectories: Sequence[TrajectoryWithRew]):
...
def rollout_stats(trajectories: Sequence[TrajectoryWithRew]) -> dict:
...
def flatten_trajectories_no_rew(
  trajectories: Sequence[TrajectoryNoRew],
) -> TransitionsNoRew:
  ...
  return TransitionsNoRew(**cat_parts)
...
def flatten_trajectories_with_rew(
  ...
  return TransitionsWithRew(**dataclasses.asdict(transitions), rews=rews)
...
def generate_transitions(policy,
  ...
  if truncate and n_timesteps is not None:
    ...
    transitions = TransitionsWithRew(**truncated)
  return transitions
...
def load(path: str) -> Sequence[TrajectoryWithRew]:
  try:
    globals()['Trajectory'] = _TrajectoryBackwardCompatible
    with open(path, "rb") as f:
      trajectories = pickle.load(f)
    if len(trajectories) > 0:
      if isinstance(trajectories[0], _TrajectoryBackwardCompatible):
        trajectories = [TrajectoryWithRew(**traj._asdict())
                        for traj in trajectories]
  finally:
    del globals()['Trajectory']
  return trajectories
