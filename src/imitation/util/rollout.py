...
class RolloutInfoWrapper(gym.Wrapper):
    """Add the entire episode's rewards and observations to `info` at episode
    end.

    Whenever done=True, `info["rollouts"]` is a dict with keys "obs" and
    "rews", whose corresponding values hold the Numpy arrays containing
...
def recalc_rewards_traj(traj: Trajectory, reward_fn: common.RewardFn,) -> np.ndarray:
    """Returns the rewards of the trajectory calculated under a diff reward
    fn."""
...
class Transitions(NamedTuple):
    ...
    Usually generated by combining and processing several Trajectories
    via `flatten_trajectories()`.
    """
...
class TrajectoryAccumulator:
    ...
    partially-completed trajectories (e.g. when rolling out a VecEnv to
    collect a set number of transitions). Each in-progress trajectory is
    identified by a 'key', which enables several independent
    trajectories to be collected at once. They key can also be left at
    its default value of `None` if you only wish to collect one
    trajectory.
    """
    ...
    def add_steps_and_auto_finish(
        ...
        """Calls `add_step` repeatedly using acts and the returns from
        `venv.step`.
...
def make_sample_until(
    ...
    """Returns a termination condition sampling until n_timesteps or
    n_episodes.
...
def mean_return(*args, **kwargs) -> float:
    ...
    Shortcut to call `generate_trajectories` and fetch the
    `rollout_stats` value for `'return_mean'`; see documentation for
    `generate_trajectories` and `rollout_stats`.
    """
...
def save(
    ...
    **kwargs,
    ...
    """Generate policy rollouts and save them to a pickled
    Sequence[Trajectory].
