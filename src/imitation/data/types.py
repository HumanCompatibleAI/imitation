...
class TransitionsMinimal:
    ...
    by the agent when choosing action `acts[i]`, but `obs[i]` is not required to
...
class Transitions(TransitionsMinimal):
    ...
    Usually generated by combining and processing several TrajectoryNoRew objects via
