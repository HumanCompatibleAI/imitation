...
from stable_baselines3.common.vec_env import VecEnvWrapper, vec_video_recorder
...
def eval_policy(
    ...
    Args:
        ...
        render_fps: If positive and `render` is True, then limit the number of steps per
            second to this number.
        save_video: If True, then save a mp4 video in the `log_dir` directory. Name of
            file will be printed to stdout. Only compatible with eval_n_timesteps.
        log_dir: The directory to log intermediate output to. (As of 2020-07-19
    ...
    if save_video:  # pragma: no cover
        # Notes: needs ffmpeg or avconv, as per
        # https://stable-baselines.readthedocs.io/en/master/guide/examples.html#record-a-video
        assert eval_n_timesteps is not None
        assert eval_n_episodes is None
        venv = vec_video_recorder.VecVideoRecorder(venv, log_dir, lambda _: True,
                                                   video_length=eval_n_timesteps)
...
def main_console():
    observer = FileStorageObserver(Path("output", "sacred", "eval_policy"))
