...
def set_tf_vars(*,
  ...
  """Set a collection of variables to take the values in `values`. Variables
  can be either specified by scope or passed directly into the function as a
  list. Variables and values will be matched based on the order in which they
  appear in their respective collections, so there must be as many values as
  variables.
...
class BCTrainer:
  ...
  def train(self, *, n_epochs=100, on_epoch_end=None):
    ...
    Args:
      ...
      on_epoch_end (function, (dict) -> None): optional callback to run at
