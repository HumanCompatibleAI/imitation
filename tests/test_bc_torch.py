...
from imitation.algorithms import bc_torch
...
def trainer(request, session, venv):
    ...
    return bc_torch.BC(venv.observation_space, venv.action_space, expert_data=data)


def test_bc(trainer: bc_torch.BC, venv):
...
def test_save_reload(trainer, tmpdir):
    ...
    new_policy = bc_torch.BC.reconstruct_policy(pol_path)
