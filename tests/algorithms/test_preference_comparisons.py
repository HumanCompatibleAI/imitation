...
def test_trajectory_dataset_seeding(cartpole_expert_trajectories: List[TrajectoryWithRew], num_samples: int = 400):
    dataset1 = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=0)
    sample1 = dataset1.sample(num_samples)
    dataset2 = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=0)
    sample2 = dataset2.sample(num_samples)

    _check_trajs_equal(sample1, sample2)

    dataset3 = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=42)
...
def test_trajectory_dataset_len(cartpole_expert_trajectories: List[TrajectoryWithRew], num_steps: int):
    dataset = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=0)
...
def test_trajectory_dataset_too_long(cartpole_expert_trajectories: List[TrajectoryWithRew]):
    dataset = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=0)
    with pytest.raises(RuntimeError, match="Asked for.*but only.* available"):
        dataset.sample(100000)


def test_trajectory_dataset_shuffle(cartpole_expert_trajectories: List[TrajectoryWithRew], num_steps: int = 400):
    dataset = preference_comparisons.TrajectoryDataset(cartpole_expert_trajectories, seed=0)
    sample = dataset.sample(num_steps)
