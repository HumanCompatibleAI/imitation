...
def test_reward_overwrite():
  ...
  wrapped_env = util.RewardVecEnvWrapper(env, reward_fn)
  policy = util.RandomPolicy(env.observation_space, env.action_space)
  default_stats = util.rollout_stats(policy, env, n_episodes=10)
  wrapped_stats = util.rollout_stats(policy, wrapped_env, n_episodes=10)
