"""Smoke tests for imitation.scripts.mountain_car_plots module."""
...
def fake_reward_fn(obs, acts, next_obs, steps):
  ...
  moving up the y-axis to dramatically increase reward and moving right to
  slightly increase reward. You can confirm this effect by calling `plt.show()`
  inside `test_smoke_make_heatmap`."""
  pos, vel = obs[:, 0], obs[:, 1]
  return vel * 100 + pos
...
def test_smoke_make_heatmap(trajs):
  ...
  for act in range(mountain_car_plots.MC_NUM_ACTS):
    fig = mountain_car_plots.make_heatmap(act, fake_reward_fn, gen_trajs=trajs,
                                          exp_trajs=trajs)
